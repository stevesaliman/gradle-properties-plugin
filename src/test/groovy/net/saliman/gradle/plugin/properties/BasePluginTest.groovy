 ...
 * <p>
 * Also note that we deal with system properties in 2 different ways; reading and writing.
 * Properties ending with "Property" are properties we read.   "read" properties are used to see if
 * the have the right order of precedence when we set the values of project properties, some of
 * which can come from system properties.  "write" properties are used to see if we set the right
 * value of system properties when we read a build property with the right prefix.
 *
...
class BasePluginTest {
    ...
    /**
     * helper method to create the projects and tasks used in the unit tests.  This will create a
     * parent project with one child project. It also sets up a spot for command properties so we
    ...
    /**
     * Helper method to set the properties that would be set before the plugin is applied.  These
     * properties would be set by Gradle itself as it creates projects from build.gradle files.
     * Gradle only handles properties in the command line and the 3 gradle.properties file, and it
     * only sets system properties from the home and parent project files.
     * @param includeParentFile whether or not to assume the parent gradle.properties file was
     *         present.
     * @param includeChildFile whether or not to assume the child gradle.properties file was
     *         present.
     */
    def setFileProperties(includeParentFile, includeChildFile) {
        ...
        // Set the Java system properties.  Gradle would have only processed the parent project file
        // and the home file.
        if ( includeParentFile ) {
    ...
    /**
     * Helper method to set the properties that Gradle would have set for us via the command line,
     * system properties, and environment variables.  Per the gradle docs, it does not set any
     * system properties from the environment or the command line.
     *
     * @param setEnvironmentProperties whether or not to set the environment properties
     * @param setSystemProperties whether or not to set the system properties
     * @param commandProperties a map of properties that should be added to the "command line" for
     * our build.
     */
    def setNonFileProperties(setEnvironmentProperties, setSystemProperties,
                             Map commandProperties) {
        // Set or clear environment variables.  We don't worry about java properties here because
        // you can't have a dot in an environment variable, and gradle doesn't set system properties
        ...
        // Set system properties.  We don't set the "prop" properties, because we don't read them in
        // the plugin, we write them..
        ...
        // 1 - Add each property to Gradle's list of command line arguments so that the plugin will
        //     process them when the plugin is applied.
        // 2 - Add the property to the project's existing properties since Gradle would have already
        //     added them to the project before applying the plugin.
    ...
    /**
     * Do the actual work of copying the test property files to the correct locations within the
     * project hierarchy
     */
    def copyFiles() {
