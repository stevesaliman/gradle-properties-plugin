...
/**
 * Test class for parent projects that apply Properties plugin.  In a multi project build, parent
 * and child projects use slightly different files.  A child project will inherit from the parent
 * project's property files, but the parent project does not use the child project's files.
 * <p>
...
class PropertiesPluginParentProjectTest extends BasePluginTest {
    ...
    // These tests are split up into multiples so that if one part works but another doesn't we have
    // an easier time finding things.
    /**
     * Prior to fixing GitHub issue #22, the plugin would attempt to convert all camel case
     * properties to dot notation (propertyName -> property.name), in the filterTokens, but this
     * made some strange, unnecessary tokens when we have a property that is all upper case
     * (PROPERTY_NAME became .p.r.o.p.e.r.t.y_.n.a.m.e).  This test makes sure we don't do that any
     * more.
     */
    @Test
    void applyUpperCommandProperty() {
        // simulate a "-PcommandProperty=Command.commandValue -PgradleUserName=user" command line
        def commandArgs = [
        ...
        // Prior to fixing Github issue #22, the plugin would create this strange, unneeded property.
        testFilter = tokens['.u.p.p.e.r_.p.r.o.p.e.r.t.y']
        assertNull(testFilter)
    }

    /**
     * Verify that a command line value overrides everything else, and that we don't set system
     * properties from it.
     */
    @Test
    void applyCommandProperty() {
        ...
        // check the property that looks like it should set a system property.  The build property
        // should be set, but per gradle docs, it should be ignored when setting a command property.
    ...
    /**
     * Verify that when a property is set in all the files, but not the command line, system
     * properties, or environment variables, the user value wins, since it is the highest priority
     * file.
     */
    @Test
    void applyUserProperty() {
        // simulate a "-PcommandProperty=Command.commandValue -PsystemProp.commandProp=commandValue
        // -PgradleUserName=user" command line
        def commandArgs = [
                commandProperty: 'Command.commandValue',
    ...
    /**
     * Verify that when a file-based property is set in all files, but no user is given, we get the
     * value from the home file.
     * */
    @Test
    void applyUserPropertyNoUser() {
        // simulate a "-PcommandProperty=Command.commandValue -PsystemProp.commandProp=commandValue"
        // command line
        def commandArgs = [
                commandProperty: 'Command.commandValue',
    ...
    /**
     * Verify that when a file-based property is set everywhere but the user file, the home file
     * wins.
     */
    @Test
    void applyHomeProperty() {
        // simulate a "-PcommandProperty=Command.commandValue -PsystemProp.commandProp=commandValue
        // -PgradleUserName=user" command line
        def commandArgs = [
                commandProperty: 'Command.commandValue',
    ...
    /**
     * Verify that when a property is set in the environment and project files, and we don't specify
     * an environment, the local environment file wins.  This also makes sure the files in the child
     * project are ignored when we apply at a parent level.
     */
    @Test
    void applyUseDefaultFile() {
        // simulate a "-PcommandProperty=Command.commandValue -PsystemProp.commandProp=commandValue
        // -PgradleUserName=user" command line
        def commandArgs = [
                commandProperty: 'Command.commandValue',
    ...
    /**
     * Verify that when a property is set in the environment and project files, we don't specify an
     * environment, but we do specify a property directory, the local environment file wins, but
     * that we use the file in the directory and not the one at the project level. This also makes
     * sure the files in the child project are ignored when we apply at a parent level.
     */
    @Test
    void applyUseDefaultFileInDirectory() {
        // simulate a "-PcommandProperty=Command.commandValue -PsystemProp.commandProp=commandValue
        // -PgradleUserName=user -PenvironmentFileName=gradle-properties" command line
        def commandArgs = [
                commandProperty: 'Command.commandValue',
                'systemProp.commandProp': 'Command.commandValue',
                gradleUserName: 'user',
    ...
    /**
     * Verify that when a property is set in the environment and project files, and we do specify an
     * environment, the specified environment file wins.  This also verifies that properties in
     * child project files are ignored.
     */
    @Test
    void applyUseAlternateFile() {
        // simulate a "-PcommandProperty=Command.commandValue -PsystemProp.commandProp=commandValue
    ...
    /**
     * Verify that when we only specify a property in the project property file, it still gets set
     * and is in the filters.  This also verifies that we ignore properties set in child project
     * files.
     */
    @Test
    ...
    // This set of tests tests what happens when certain files are missing.  To be thorough, we'll
    // test all the properties and tokens.

    /**
     * Test applying the plugin when we have no user file, but we didn't specify a user.  This is
     * not an error.  Remember that child files should not be processed.
     */
    @Test
    ...
    void applyMissingSpecifiedUserFile() {
        // simulate a "-PcommandProperty=Command.commandValue -PgradleUserName=dummy" command line
        def commandArgs = [
                commandProperty: 'Command.commandValue',
    ...
    /**
     * Test what happens when we have no home file.  This should not produce an error.
     */
    @Test
    ...
    /**
     * Test what happens when we have no environment file, and we're using the default "local" file.
    ...
    /**
     * Test what happens when we have no environment file, but we specify an environment file.  This
     * should be an error.
     */
    @Test
    ...
    /**
     * Test what happens when we have no environment file and we specify an environment file, but we
     * tell the plugin to ignore missing files.  This should behave the same as a missing local
     * environment file; our "environment" properties should come from the "project" file.
     */
    @Test
    ...
    void applyMissingProjectFile() {
        ...
        // 18 that we're checking and 12 from system properties, since we'll be missing the
        // properties that were only set in the parent project file.
