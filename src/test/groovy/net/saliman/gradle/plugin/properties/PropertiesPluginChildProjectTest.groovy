...
/**
 * Test class for child projects that apply Properties plugin.  In a multi project build, parent and
 * child projects use slightly different files.  A child project will inherit from the parent
 * project's property files, but the parent project does not use the child project's files.  This
 * test has some more to it than the parent test because the parent test doesn't deal with child
 * properties or project inheritance.
 * <p>
 * Each test also applies the plugin to the parent project before applying it to the child project.
 * This simulates what Gradle itself does, but also tests a fix for a plugin bug that was caused by
 * the fact that some Gradle API methods inherit from parent projects, and others don't and we want
 * to make sure child extension properties are set even if they existed in the parent. This is also
 * why we test for {@code childProject.environmentName} (which inherits), and
 * {@code childProject.ext.environmentName} (which doesn't).
 * <p>
 * Note that we set properties in each test because we can't seem to clear a property once it is
 * set, and Gradle itself reads some of the properties before we apply the plugin.
 * <p>
 * The property files are set up so that there is a property representing each file.  Each file sets
 * the property for that file, plus all the files below it (with more precedence) in the sequence.
 * That way, when we don't have (or don't use) a file, the values from that file should be set by
 * the file one level up.
 *
 * @author Steven C. Saliman
 */
class PropertiesPluginChildProjectTest extends BasePluginTest {
    /**
     * Set up the test data.  This calls a helper method to create the projects because we need to
     * repeat the setup in one of the tests.
     */
    @Before
    void setUp() {
        createProjects()
    ...
    // This set of tests focuses on the different ways the user can invoke the properties plugin.

    /**
     * Verify that each property came from the correct file.  In this case, all files exist, and we
     * specify a user and environment, environment variables are set, system properties are set, and
     * we have a command line value.
     */
    @Test
    void applyPluginWithAll() {
        // simulate a "-PcommandProperty=Command.commandValue -PsystemProp.commandProp=commandValue
        ...
        childProject.apply plugin: 'net.saliman.properties'
        ...
        // Check the system properties. Remember, system properties are not set from command line
        // properties, and values from child project files are ignored.
    ...
    /**
     * Apply the plugin with everything except for a command line property.  This is the same as the
     * last test, except that the command property should come from the system properties.
     */
    @Test
    ...
    /**
     * Apply the plugin with everything except for system properties.  In this case, the property
     * normally set from system properties will inherit the value from the environment variables.
     */
    @Test
    ...
    /**
     * Apply the plugin with everything except for environment variables.  In this case, the
     * property that is normally set by environment variables will inherit the value from the User
     * file.
     */
    @Test
    ...
    /**
     * Test applying the plugin when no user is given. In this case, the property that usually comes
     * from the user file will inherit the value from the home file.
     */
    @Test
    void applyNoUser() {
        // simulate a "-PcommandProperty=Command.commandValue -PsystemProp.commandProp=commandValue
    ...
    /**
     * Apply the plugin when no environment is specified.  This should apply environment properties
     * from the Local file instead of the Test file.  This will also verify that the child
     * environment files override the parent environment files.
     */
    @Test
    ...
    /**
     * Apply the plugin when no environment is specified, but we do specify a directory.  This
     * should apply environment properties from the Local file instead of the Test file, and it
     * should use the one in the property directory.  This will also verify that the child
     * environment files override the parent environment files.
     */
    @Test
    void applyUseDefaultEnvironmentFileInDirectory() {
        // simulate a "-PcommandProperty=Command.commandValue -PsystemProp.commandProp=commandValue
        // -PgradleUserName=user -PenvironmentFileDir=gradle-properties" command line
        def commandArgs = [
                commandProperty: 'Command.commandValue',
                'systemProp.commandProp': 'Command.commandValue',
                gradleUserName: 'user',
    ...
    /**
     * Test applying the plugin when we have no user file, but we didn't specify a user.  This is
     * not an error.
     */
    @Test
    void applyMissingUnspecifiedUserFile() {
        ...
        // simulate a "-PcommandProperty=Command.commandValue" -PsystemProp.commandProp=commandValue
        // command line
        def commandArgs = [commandProperty         : 'Command.commandValue',
                           'systemProp.commandProp': 'Command.commandValue']
        setNonFileProperties(true, true, commandArgs)
    ...
    /**
     * Test applying the plugin when we have no user file for a specified user.  This should produce
     * an error
     */
    @Test
    void applyMissingSpecifiedUserFile() {
        ...
        try {
            // Only apply the child because we want to make sure the error happens on the child
            // project.
            childProject.apply plugin: 'net.saliman.properties'
    ...
    /**
     * Test what happens when we have no home file.  This should not produce an error, but the home
     * property should be inherited from the child environment file.
     */
    @Test
    ...
    /**
     * Test applying the plugin when we are missing the child environment file, but we have the
     * parent environment file.  In this case, we should still get the property set in the parent
     * environment file, but the property usually set in the child environment file should come from
     * the child project file, which has precedence.
     */
    @Test
    ...
    /**
     * Test applying the plugin when we are missing the parent environment file, but we have the
     * child environment file.  In this case, the property that would have been set in the parent
     * environment file will have the value from the parent project file.
     */
    @Test
    void applyMissingParentEnvFile() {
        ...
        // We shouldn't be able to apply to the parent, but the child should run just fine.
        try {
            parentProject.apply plugin: 'net.saliman.properties'
    ...
    /**
     * Test what happens when we have no environment file in either project, but we specify an
     * environment file - in other words, we specified an invalid environment.  This should be an
     * error.
     */
    @Test
    void applyMissingSpecifiedEnvFile() {
        // simulate a "-PcommandProperty=Command.commandValue -PenvironmentName=dummy
        // -PgradleUserName=user" command line
        def commandArgs = [
                commandProperty: 'Command.commandValue',
    ...
    /**
     * Test what happens when we have no environment file in either project, but we specify an
     * environment file - in other words, we specified an invalid environment.  This should act the
     * same as the local environment, except that we'll get "local" values from the default file.
     */
    @Test
    void applyIgnoreMissingSpecifiedEnvFile() {
        // simulate a "-PpropertiesPluginIgnoreMissingEnvFile=true
        // -PcommandProperty=Command.commandValue -PenvironmentName=dummy -PgradleUserName=user"
        // command line
        def commandArgs = [
        ...
        // Only apply the child because we're only concerned with the child in this test.
        childProject.apply plugin: 'net.saliman.properties'
        ...
        // 22 that we're checking, 14 from system properties, and 2 from our setting of the override
        // property.
        def tokens = childProject.filterTokens;
    ...
    /**
     * Test what happens when we have no environment file in either project, but we didn't specify
     * an environment.  This should not be an error, but the values that would have come from
     * environment files will come instead from the project files.
     */
    @Test
    ...
    /**
     * Test what happens when we have no gradle.properties in the parent project, but we do have one
     * in the child project.  In this case, we won't ever get the property that should be set in the
     * parent project file, but we'll get the rest of them.
     */
    @Test
    void applyMissingParentProjectFile() {
        ...
        // we can't unset a property once it has been set, so redo the setup, skipping the project
        // property since Gradle would not have set it when the project file is missing.
        createProjects()
    ...
    /**
     * Test applying the plugin when we are missing the gradle.properties file in the child project,
     * but we have one in the parent project.  In this case, the property that usually gets set in
     * the child project file will come from the parent project's environment file.
     */
    @Test
    ...
    /**
     * Test what happens when we have no gradle.properties in the either project. In this case, we
     * won't ever get the property that should be set in the parent project file, but we'll still
     * get the child project property from the parent environment file.
     */
    @Test
