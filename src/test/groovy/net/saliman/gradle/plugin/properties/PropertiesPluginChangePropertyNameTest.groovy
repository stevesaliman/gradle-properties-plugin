...
/**
 * The {@link PropertiesPluginParentProjectTest} class tests applying the plugin with the standard
 * {@code environmentName} and {@code gradleUserName} properties driving the environment name and
 * gradle user.
 * <p>
 * This class tests changing these properties via {@code propertiesPluginEnvironmentNameProperty}
 * and {@code propertiesPluginGradleUserNameProperty} properties before applying the plugin to make
 * sure we still do the right thing when the user wants to reconfigure what the plugin uses during
 * apply time.
 *
...
class PropertiesPluginChangePropertyNameTest extends BasePluginTest {
     ...
     * works.  We don't need to get fancy here, the default environment works fine.  We only need to
     * check that one property was read from the local file.
     */
    @Test
    ...
    /**
     * Try applying the plugin when we specify a different property for the property file directory,
     * but we don't specify a value for that variable.  The plugin should assume the project
     * directory.  We'll try to confuse the plugin by setting the standard environmentFileDir
     * property to a different value to make sure the plugin is using the right property.  We don't
     * need to check every property from the file - we already know that works.
     */
    @Test
    void applyChangedPropFileDirPropertyNoValue() {
        ...
        // ... but the property values should come from the local file in the project directory.
        assertEquals('ParentEnvironmentLocal.parentEnvironmentValue', parentProject.parentEnvironmentProperty)
    ...
    /**
     * Try applying the plugin when we specify a different property for the property file directory,
     * and we specify a value for that variable that doesn't exist.  The plugin should fail, even if
     * the "environmentFileDir" is still set to a valid directory.
     */
    @Test
    ...
    /**
     * Try applying the plugin when we specify a different property for the property file directory,
     * and we specify a value for that variable that points to a file.  The plugin should fail, even
     * if the "environmentFileDir" is still set to a valid directory.
     */
    @Test
    ...
    /**
     * Try applying the plugin when we specify a different property for the property file directory,
     * and we specify a value for that variable that points to a valid directory.  The plugin should
     * succeed, even if "environmentFileDir" is invalid.
     */
    @Test
    void applyChangedEnvironmentFileDirPropertyGoodValue() {
        // simulate a "-PpropertiesPluginEnvironmentFileDirProperty=myDir -PmyDir=test
        // -PenvironmentFileDir=dummy" command line
        def commandArgs = [
                propertiesPluginEnvironmentFileDirProperty: 'myDir',
    ...
    /**
     * Try applying the plugin when we specify a different property for the environment name, but we
     * don't specify a value for that variable.  The plugin should assume "local".  We'll try to
     * confuse the plugin by setting the standard environment property to a different value to make
     * sure the plugin is using the right property.  We don't need to check every property from the
     * file - we already know that works.
     */
    @Test
    ...
    /**
     * Try applying the plugin when we specify a different property for the environment name, and we
     * specify a value for that variable that doesn't have a file.  The plugin should fail, even if
     * the "environmentName" is still set to a valid file.
     */
    @Test
     ...
     * Try applying the plugin when we specify a different property for the environment name, and we
     * specify a value for that variable that doesn't have a file.  The plugin should fail, even if
     * the "environmentName" is still set to a valid file, and even though there is a file for the
     * "environmentName", we should get our values from the project file because we're using a
     * different property to specify the environment.
     */
    @Test
    void applyChangedEnvNamePropertyIgnoreBadValue() {
        // simulate a "-PpropertiesPluginIgnoreMissingEnvFile=true
        // -PpropertiesPluginEnvironmentNameProperty=myEnvironment -PmyEnvironment=dummy
        // -PenvironmentName=test" command line
        def commandArgs = [
    ...
    /**
     * Try applying the plugin when we specify a different property for the environment name, and we
     * specify a value for that variable that does have have a file.  The plugin should succeed,
     * even if "environmentName" is invalid.
     */
    @Test
    ...
    /**
     * Try applying the plugin when we specify a different property for the user name, but we don't
     * specify a value for that variable.  The plugin should assume no value, and we should get the
     * "user" property from the "home" file.  We'll try to confuse the plugin by setting the
     * standard user property to a different value to make sure the plugin is using the right
     * property.  We don't need to check every property from the file - we already know that works.
     */
    @Test
    void applyChangedUserNamePropertyNoValue() {
        // simulate a "-PpropertiesPluginGradleUserNameProperty=myUser -PgradleUserName=dummy"
        // command line
        def commandArgs = [
    ...
    /**
     * Try applying the plugin when we specify a different property for the user name, and we
     * specify a value for that variable that doesn't have a file.  The plugin should fail, even if
     * the "gradleUserName" is still set to a valid file.
     */
    @Test
    void applyChangedUserNamePropertyBadValue() {
        // simulate a "-PpropertiesPluginGradleUserNameProperty=myUser -PmyUser=dummy
     ...
     * Try applying the plugin when we specify a different property for the user name, and we
     * specify a value for that variable that does have have a file.  The plugin should succeed,
     * even if "gradleUserNAme" is invalid.
     */
    @Test
    void applyChangedUserNamePropertyGoodValue() {
        // simulate a "-PpropertiesPluginGradleUserNameProperty=myUser -PmyUser=user
