 ...
 * project, it uses the values of the {@code environmentName} and {@code gradleUserName} properties
 * to reload all the project properties in the following order:<br>
 * <ol>
 * <li>
 * gradle.properties in the parent project's directory, if the project is a module of a
 * multi-project build.
 * </li>
 * <li>
 * gradle-${environmentName}.properties in the parent project's directory, if the project is a
 * module of a multi-project build. If no environment name is specified, the default is "local".
 * </li>
 * <li>
 * gradle.properties in the project directory
 * </li>
 * <li>
 * gradle-${environmentName}.properties in the project directory. If no environment name is
 * specified, the default is "local".
 * </li>
 * <li>
 * gradle.properties in the user's ${gradleUserHomeDir} directory.
 * </li>
 * <li>
 * If the ${gradleUserName} property is set, gradle-${gradleUserName}.properties in the user's
 * ${gradleUserHomeDir} directory.
 * </li>
 * <li>
 ...
 * </ol>
 * <p>
 * When the plugin is applied to a {@code Settings} object, it uses the {@code environmentName} and
 * {@code gradleUserName} properties to reload all the settings properties in the following
 * order:<br>
 * <ol>
 * <li>
 ...
 * </li>
 * <li>
 * gradle-${environmentName}.properties in the directory where settings.gradle is located. If no
 ...
 * </ol>
 * <p>
 * As mentioned, this plugin looks for the gradle-${environmentName}.properties file in the project
 * directory for projects, or the directory with settings.gradle if the plugin is applied to a
 * Settings object.  You can change this behavior by specifying a value for the
 * {@code environmentFileDir} property.  It is strongly recommended to use a subdirectory of the
 * project or settings directory when using this option.  This property only effects where the
 * environment specific files are located, it does not change where the plugin looks for the other
 * files, such as the user files or default gradle files.
 * <p>
 * {@code environmentFileDir}, {@code environmentName}, and {@code gradleUserName} are the
 * properties that this plugin use by default to get the location of the environment files and the
 * names of the environment and gradle user to use.  If you don't like these names, or if there is a
 * clash with properties you already use in your build, you can configure the properties the plugin
 * will use.  You can tell the plugin what property to use for the environment file directory by
 * setting a value for the {@code propertiesPluginEnvironmentFileDirProperty} property. You can tell
 * the plugin what property to use for the name of the environment by setting the
 * {@code propertiesPluginEnvironmentNameProperty}, and you can tell the plugin what property to use
 * to set the user name by setting a value for the {@code propertiesPluginGradleUserNameProperty}
 * property. Those properties have to be set before this plugin is applied. That means you can put
 * them in the standard property file locations supported by Gradle itself, in environment
 * variables, system properties, -P options or in the build.gradle file itself before applying this
 * plugin.
 * <p>
 * The last thing to set a property wins.  All files are optional unless an environment or user is
 * specified, in which case the file belonging to the specified environment or user must exist.  If
 * an environment file directory is specified, that directory must exist, be a directory, and be
 * readable.
 * <p>
 * As properties are set, they are also placed in a filterTokens property.  The filterTokens
 * property is a map of tokens that can be used when doing a filtered file copy. There will be a
 * token for each property defined by one of the listed methods. The name of each token will be the
 * name of the property from the properties file, after converting the camel case property name to
 * dot notation. For example, if you have a myPropertyName property in the file, the plugin will
 * create a my.property.name filter token, whose value is the property's value. The original camel
 * case name will also be added as token.
 * <p>
 * By default, this plugin fails the build if you specify an environment, but no file exists for
 * that environment.  You can use the {@code propertiesPluginIgnoreMissingEnvFile} property to
 * override this behavior.
 * <p>
 * Finally, the properties plugin also adds some properties to every task in your build:
 * <p>
 * {@code requiredProperty} and {@code requiredProperties} can be used to define what properties
 * must be present for that task to work.  Required properties will be checked after configuration
 * is complete, but before any tasks are executed.
 * <p>
 * {@code recommendedProperty} and {@code recommendedProperties} can be used to define properties
 * that the task can work without, but it (or the deployed application) will use a default value for
 * the property. The value of this property is that we can prompt new developers to either provide a
 * property, or make sure default config files are set up correctly.
 * <p>
 * Special thanks to Hans Dockter at Gradleware for showing me how to attach to the right place in
...
class PropertiesPlugin implements Plugin<PluginAware> {
     ...
     * {@code Project} and {@code Settings} objects.
     *
     * @param the object to which this plugin should be applied.  Currently, this must be either a
     *         {@code Project} or {@code Settings} object.
     */
     ...
     *
     * @param pluginAware the project or settings object to which we are applying the plugin.
     * @param buildPropertyFileList a function that takes a PluginAware object and an environment
     *         name, and returns a list of property files to evaluate when the plugin is applied.
     */
    private doApply(pluginAware, propertyFileListBuilder) {
        // We need to use pluginAware.ext.has and not pluginAware.hasProperty because we do not want
        // to find properties in parent projects.
        ...
        // If the user hasn't set a property file directory, assume the project directory.
        ...
        // process files from least significant to most significant. With gradle properties, Last
        // one in wins.
        ...
        if ( envName != 'local' && !foundEnvFile ) {
            // Fail the build unless the propertiesPluginIgnoreMissingEnvFile property is set to true.
    ...
    /**
     * Build a list of property files to process for a project, in the order in which they need to
     * be processed.
     *
    ...
    private buildPropertyFileListFromProject(project, envFileDir, envName) {
        ...
        def files = []
        while ( p != null ) {
            // We'll need to process the files from the top down, so build the list backwards.  Note
            // that only the environment file can come from a special location.  gradle.properties
            // must come from the project itself.
            ...
            // Whether or not the project property files can contain system properties is entirely
            // dependent on whether the project whose files we're currently processing is the root
            // project.
    ...
    /**
     * Build a list of property files to process for a settings object, in the order in which they
    ...
    /**
     * Helper method that adds the files common to both projects and settings to the end of the
     * property file list.
     *
    ...
    private addCommonPropertyFileList(pluginAware, files) {
        // Add the rest of the files to the end.  The user property file is optional...
    ...
    /**
     * Process the environment properties, setting pluginAware properties and adding tokens for any
     * environment variable starting with {@code ORG_GRADLE_PROJECT_}, per the Gradle specification.
     *
     * @param pluginAware the enclosing pluginAware.
     */
    ...
    /**
     * Process the system properties, setting properties and adding tokens for any system property
     * starting with {@code org.gradle.project.}, per the Gradle specification.
     * @param pluginAware the enclosing pluginAware.
     */
    ...
    /**
     * Register a task listener to add the property checking methods to all current tasks as well as
     * tasks that are added to the project after the plugin is applied.
     *
     * @param project the project applying the plugin
     */
    private registerTaskListener(project) {
        ...
        project.tasks.all { task ->
            ...
            task.ext.requiredProperty = { String propertyName ->
                // ... but we don't want to execute at configuration time.  We want to record that
                // the property is needed for the task, and check it when the graph is ready,
                // between configuration and execution. the whenReady method allows us to pass in a
                // closure to be executed...
                project.gradle.taskGraph.whenReady { graph ->
                    // ... But we only want to actually do it if the task needing the property is
    ...
    /**
     * Helper method to check a recommended property and print a warning if it is missing.
     *
     * @param project the project we're dealing with
     * @param propertyName the name of the property we want to check
     * @param task the task checking the property.
    ...
    /**
     * helper method to convert a camel case property name to a dot notated one.  This is used by
     * the plugin for filtering.  For example, myPropertyName would become my.property.name as a
     * token.  Property names that don't start with a lower case letter are assumed to not be camel
     * case and are returned as is.  This means that callers will add the same property twice, which
     * should be fine.
     *
